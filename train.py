...
def parse():
    ...
    parser.add_argument('--data', type=str, default='data/WebOfScience')
    ...
    parser.add_argument('--checkpoint', type=str, default='checkpoint_last.pt', help='Checkpoint file to resume from')
    return parser
...
if __name__ == '__main__':
    ...
    # Check if the PyTorch build has CUDA support
    print(f"PyTorch built with CUDA: {torch.cuda.is_available() or '+cu' in torch.__version__}")
    
    # Find PyTorch installation paths
    torch_path = os.path.dirname(torch.__file__)
    print(f"\nPyTorch installation path: {torch_path}")
    
    # Look for multiple PyTorch installations in site-packages
    site_packages = [p for p in sys.path if 'site-packages' in p]
    torch_installs = []
    
    for site_path in site_packages:
        potential_torch_paths = glob.glob(os.path.join(site_path, 'torch*'))
        for path in potential_torch_paths:
            if os.path.isdir(path) and os.path.basename(path).startswith('torch'):
                torch_installs.append(path)
    
    print(f"\nFound {len(torch_installs)} torch-related directories:")
    for i, path in enumerate(torch_installs, 1):
        print(f"{i}. {path}")
    
    # Check for CPU and CUDA libraries
    lib_path = os.path.join(torch_path, 'lib')
    if os.path.exists(lib_path):
        cpu_libs = glob.glob(os.path.join(lib_path, '*cpu*.so'))
        cuda_libs = glob.glob(os.path.join(lib_path, '*cuda*.so'))
        print("cpu_libs: ", cpu_libs)
        print("cuda_libs: ", cuda_libs)
        print(f"\nFound {len(cpu_libs)} CPU-specific libraries")
        print(f"Found {len(cuda_libs)} CUDA-specific libraries")
        
        if len(cpu_libs) > 0 and len(cuda_libs) > 0:
            print("WARNING: Both CPU and CUDA libraries found, which may cause conflicts!")
    ...
    if args.model == 'prompt':
        if os.path.exists(os.path.join(data_path, args.model)):
            print("exists path: ", os.path.join(data_path, args.model))
            dataset = datasets.load_from_disk(os.path.join(data_path, args.model))
        else:
            dataset = datasets.load_dataset('json',
                                            data_files={'train': data_path +'train.json'.format(args.data, args.data),
                                                        'dev': data_path +'dev.json'.format(args.data, args.data),
                                                        # 'test': data_path +'test.json'.format(args.data, args.data), 
                                                        })
            ...
            def data_map_function(batch, tokenizer):
                ...
                for l, t in zip(batch['labels'], batch['text']):
